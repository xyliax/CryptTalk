...
pub fn ui(f: &mut Frame, app: &App, center_chunk: &Rect, bottom_chunk: &Rect) {
    ...
    let main_chunks = Layout::default()
        ...
        .constraints([
            Constraint::Length(3), // "LOGIN PAGE"
    ...
    // "LOGIN PAGE"
    f.render_widget(
        Paragraph::new("LOGIN PAGE".green().bold())
            .block(
                Block::default()
    ...
    f.render_widget(
        Paragraph::new("LOGIN".blue().bold())
            ...
            .alignment(Alignment::Center),
        centered_rect(20, 100, main_chunks[5]),
    );
    // bottom bar
    f.render_widget(
        Paragraph::new(app.notice.clone().green().bold())
            .block(
                Block::default()
                    .title(Title::from("Logging".yellow()))
